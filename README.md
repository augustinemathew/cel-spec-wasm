...
`//examples:examples_smoke_test` on every `bazel test` sweep, so the
code you read here is code that executes. A `Program` is pure bytes: compile
...
| | |
| --- | --- |
| **Passing** | **1972 / 2454** of the upstream CEL conformance corpus (80.4%) |
| Of expressions attempted | **1972 / 1973 = 99.9%** (excludes the by-design skips below) |
| Intentional skips | 481 — DYN/dynamic typing (227, out of scope by design), check-disabled rows (144), scope-not-yet-shipped (110: extensions, check-only, spec/type-env edges) |
| Failing | 1 — `optional.ofNonZeroValue(<message>)` traps at eval (cleanup-backlog #10) |

Live per-fixture breakdown (autogenerated):
...
`optionals` (25/70; the rest need DYN).
