...
#define BENCH(label, expr)                                         \
  ...
  BENCH("i64 x5", bench_abi_engine_i64_5((int64_t)i, (int64_t)i, (int64_t)i,
                                         (int64_t)i, (int64_t)i));
  ...
  BENCH("bytes x1", bench_abi_engine_bytes_1(&by));
  {
    uint64_t t0 = now_ns();
    uint64_t a = 0;
    runner_world_string_t ret;
    for (long i = 0; i < N; i++) {
      bench_abi_engine_str_3_ret(&s1, &s2, &s3, &ret);
      a += ret.len;
      runner_world_string_free(&ret);
    }
    uint64_t t1 = now_ns();
    g_sink += a;
  ...
  for (size_t k = 0; k < 6; k++) {
    size_t L = lens[k];
    runner_world_string_t a{(uint8_t*)buf, L}, b{(uint8_t*)buf, L},
        c{(uint8_t*)buf, L};
    uint64_t t0 = now_ns();
    uint64_t acc2 = 0;
    for (long i = 0; i < N; i++) {
      acc2 += bench_abi_engine_str_3(&a, &b, &c);
    }
    uint64_t t1 = now_ns();
    g_sink += acc2;
    char lab[32];
    snprintf(lab, sizeof lab, "L=%zu (x3)", L);
