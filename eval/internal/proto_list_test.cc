...
TEST(ProtoListTest, RepeatedInt32Hits) {
  ...
  PL_ASSIGN_OR_ASSERT(v, pl.At(1, celwasm::CelType::Int()));
...
TEST(ProtoListTest, RepeatedStringHits) {
  ...
  PL_ASSIGN_OR_ASSERT(v, pl.At(0, celwasm::CelType::String()));
...
TEST(ProtoListTest, RepeatedBoolHits) {
  ...
  PL_ASSIGN_OR_ASSERT(v0, pl.At(0, celwasm::CelType::Bool()));
  EXPECT_EQ(*v0.AsBool(), true);
  PL_ASSIGN_OR_ASSERT(v1, pl.At(1, celwasm::CelType::Bool()));
...
TEST(ProtoListTest, RepeatedDoubleHits) {
  ...
  PL_ASSIGN_OR_ASSERT(v, pl.At(1, celwasm::CelType::Double()));
...
TEST(ProtoListTest, RepeatedTimestampPeelsToTimestamp) {
  ...
  PL_ASSIGN_OR_ASSERT(v, pl.At(0, celwasm::CelType::Timestamp()));
  EXPECT_EQ(v.kind(), celwasm::Value::Kind::kTimestamp);
}
...
TEST(ProtoListTest, EmptyRepeatedReadsAsZeroSize) {
  ...
  PL_ASSIGN_OR_ASSERT(v, pl.At(0, celwasm::CelType::Int()));
...
TEST(ProtoListTest, ForEachVisitsAllInOrder) {
  ...
  pl.ForEach([&](const celwasm::Value& v) {
...
TEST(ProtoBackingListTest, ReadFieldRepeatedReturnsHostList) {
  ...
  PL_ASSIGN_OR_ASSERT(v, pb.ReadField(/*field_number=*/18, "rep_i32",
                                      celwasm::CelType::Int()));
  ASSERT_EQ(v.kind(), celwasm::Value::Kind::kList);
