...
celwasm::Value ThreeInts() {
  std::vector<celwasm::Value> elements;
  elements.emplace_back(celwasm::Value::Int(10));
  elements.emplace_back(celwasm::Value::Int(20));
  elements.emplace_back(celwasm::Value::Int(30));
...
TEST(ValueListTest, ListBuilderProducesKListKind) {
  celwasm::Value l = ThreeInts();
  EXPECT_EQ(l.kind(), celwasm::Value::Kind::kList);
...
TEST(ValueListDeathTest, HostListNullBackingFires) {
  EXPECT_DEATH(
      { (void)celwasm::Value::HostList(nullptr); }, "must not be null");
}
...
TEST(ValueListTest, StructurallyEqualsIsPointerIdentity) {
  ...
  celwasm::Value a = celwasm::Value::HostList(backing);
  celwasm::Value b = celwasm::Value::HostList(backing);
  EXPECT_TRUE(a.StructurallyEquals(b));

  celwasm::Value c = ThreeInts();  // distinct backing
...
TEST(HostListTest, AtReturnsValueOnHit) {
  std::vector<celwasm::Value> elements;
  elements.emplace_back(celwasm::Value::Int(7));
  elements.emplace_back(celwasm::Value::String("hello"));
  HostList l(std::move(elements));

  HL_ASSIGN_OR_ASSERT(v0, l.At(0, celwasm::CelType::Int()));
  EXPECT_EQ(*v0.AsInt(), 7);

  HL_ASSIGN_OR_ASSERT(v1, l.At(1, celwasm::CelType::Int()));
  EXPECT_EQ(*v1.AsString(), "hello");
}

TEST(HostListTest, AtAtEqualsCountReturnsOutOfBoundsError) {
  HostList l({celwasm::Value::Int(1)});
  HL_ASSIGN_OR_ASSERT(v, l.At(1, celwasm::CelType::Int()));
...
TEST(HostListTest, AtFarPastCountReturnsOutOfBoundsError) {
  HostList l({});  // empty
  HL_ASSIGN_OR_ASSERT(v, l.At(999, celwasm::CelType::Int()));
  EXPECT_EQ(v.kind(), celwasm::Value::Kind::kError);
  HL_ASSIGN_OR_ASSERT(e, v.ErrorInfo());
  EXPECT_EQ(e->code, celwasm::ErrorCode::kIndexOutOfBounds);
}
...
struct ElementCase {
  const char* name;
  celwasm::Value (*make)();
  std::function<void(const celwasm::Value&)> verify;
};
...
TEST_P(HostListElementRoundTripTest, AtPosZeroHits) {
  ...
  HostList l({c.make()});
  HL_ASSIGN_OR_ASSERT(v, l.At(0, celwasm::CelType::Int()));
  c.verify(v);
}

INSTANTIATE_TEST_SUITE_P(
    AllElementKinds, HostListElementRoundTripTest,
    ::testing::Values(ElementCase{"null", &celwasm::Value::Null,
                                  [](const celwasm::Value& v) {
                                    EXPECT_TRUE(v.IsNull());
                                  }},
                      ElementCase{"bool",
                                  +[] {
                                    return celwasm::Value::Bool(true);
                                  },
                                  [](const celwasm::Value& v) {
                      ...
                      ElementCase{"double",
                                  +[] {
                                    return celwasm::Value::Double(3.5);
                                  },
                                  [](const celwasm::Value& v) {
                      ...
                      ElementCase{"string",
                                  +[] {
                                    return celwasm::Value::String("abc");
                                  },
                                  [](const celwasm::Value& v) {
...
TEST(HostListTest, ForEachVisitsAllInOrder) {
  HostList l({celwasm::Value::Int(1), celwasm::Value::Int(2),
              celwasm::Value::Int(3)});
  std::vector<int64_t> seen;
  l.ForEach([&](const celwasm::Value& v) {
