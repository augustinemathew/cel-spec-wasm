...
```
total=2454  pass=1373 (55.9%)  skip=955 (38.9%)  fail=126 (5.1%)
```
...
`pass / skip / fail` per fixture, sorted by pass rate.  Regenerate
by running `run_conformance` and copying its per-fixture output;
the harness emits a `skip-by-category` line per fixture
automatically.

| Fixture | Total | Pass | Skip | Fail | Pass% | Skip categories |
|---|---:|---:|---:|---:|---:|---|
| `timestamps.textproto`      |  76 |  75 |   0 |   1 | 99% | — |
| `fp_math.textproto`         |  30 |  29 |   1 |   0 | 97% | disable_check=1 |
| `integer_math.textproto`    |  64 |  61 |   3 |   0 | 95% | disable_check=3 |
| `conversions.textproto`     | 109 | 102 |   2 |   5 | 94% | disable_check=1 static_subset=1 |
| `bindings_ext.textproto`    |   8 |   7 |   0 |   1 | 88% | — |
...
| `macros2.textproto`         |  46 |  39 |   7 |   0 | 85% | static_subset=7 |
| `lists.textproto`           |  39 |  34 |   3 |   2 | 87% | static_subset=3 |
| `parse.textproto`           | 219 | 181 |  18 |  20 | 83% | disable_check=17 type_env=1 |
| `plumbing.textproto`        |   5 |   4 |   1 |   0 | 80% | disable_check=1 |
| `string.textproto`          |  51 |  40 |   9 |   2 | 78% | compile_unimpl=9 |
| `proto3.textproto`          |  85 |  62 |  13 |  10 | 73% | disable_check=6 static_subset=7 |
| `logic.textproto`           |  30 |  21 |   9 |   0 | 70% | disable_check=9 |
| `enums.textproto`           |  85 |  55 |   2 |  28 | 65% | disable_check=2 |
| `proto2.textproto`          | 118 |  67 |  43 |   8 | 57% | disable_check=6 envelope=18 static_subset=19 |
| `dynamic.textproto`         | 226 | 106 |  92 |  28 | 47% | disable_check=20 static_subset=72 |
| `namespace.textproto`       |  14 |   6 |   4 |   4 | 43% | disable_check=4 |
| `fields.textproto`          |  60 |  26 |  28 |   6 | 43% | disable_check=5 static_subset=15 type_env=8 |
| `type_deduction.textproto`  |  47 |  20 |  25 |   2 | 43% | check_only=25 |
| `wrappers.textproto`        |  36 |   9 |  18 |   9 | 25% | static_subset=18 |
| `math_ext.textproto`        | 199 |   0 | 199 |   0 |  0% | envelope=83 ext_unimpl=116 |
| `string_ext.textproto`      | 216 |   0 | 216 |   0 |  0% | disable_check=44 envelope=78 ext_unimpl=94 |
...
| `proto2_ext.textproto`      |  18 |   0 |  18 |   0 |  0% | envelope=18 |
| `encoders_ext.textproto`    |   4 |   0 |   4 |   0 |  0% | ext_unimpl=4 |
| `unknowns.textproto`        |   0 |   0 |   0 |   0 |  —  | (empty fixture) |

Corpus-wide SKIP totals:

| Category | Count | Disposition |
|---|---:|---|
| `ext_unimpl`     | 390 | Scope-not-shipped — extensions pass (the largest single bucket; covers all of `math_ext` / `string_ext` / `optionals` / `network_ext` / `block_ext` / `encoders_ext` plus a handful of ext-shaped rows scattered through proto2/proto3/wrappers). |
| `envelope`       | 197 | Scope-not-shipped — matcher kind not handled (mostly `type_value` matchers for ext-libs and `proto2_ext` extension-field accessor rows). |
...
| `compile_unimpl` |   9 | Scope-not-shipped — `matches` regex (the only Compile-stub still in the corpus). |
| **Total**        | **955** | |

Of the 955 SKIPs: ~322 are out-of-scope by design
(`disable_check` + `static_subset`); the rest (633) are
