// M13 Probe 3 (Rust) — Rust foreign wasm module honours the
// cross-language ABI.
//
// Sibling to m13_p2_test.cc / m13_p3_c_test.cc.  Same caller WAT.
// Same assertions.  Only the foreign module changes — built by
// `rules_rust/build_rules.sh` from `rules_rust/rules.rs` via
// rustup's stable rustc with `--target=wasm32-unknown-unknown`.
//
// What this proves on top of Probe 2 + Probe 3 (C):
//
//   * A third toolchain (Rust no_std) honours the cross-language
//     ABI cleanly — no language-specific accommodation needed in
//     the host harness.
//   * Rust's `wasm32-unknown-unknown` target shapes the wasm
//     similarly to bare C: defines memory, no imports, no
//     `_initialize` export.  The engine adapter (in production,
//     `Engine::Instantiate`) covers both the with- and without-
//     `_initialize` paths.
...
TEST(M13Probe3Rust, RustForeignWasmCallReturnsBool) {
  WasmtimeState s;
  ...
  const std::string rules_bytes = ReadWorkspaceFile(
      "compiler_v2/probes/m13_custom_fns/rules_rust/rules.wasm");
  ASSERT_FALSE(rules_bytes.empty())
      << "could not read rules_rust/rules.wasm — did rules_rust/build_rules.sh run?";
  ...
  // Rust no_std defaults: no imports, no _initialize.  Empty linker.
  ...
  EXPECT_EQ(out.kind, static_cast<uint32_t>(CEL_BOOL))
      << "Rust `allow` should have written a CEL_BOOL CelValue";
  EXPECT_EQ(out.payload.b, 1)
      << "Rust `allow` should have returned true for two CEL_STRING args";
}
