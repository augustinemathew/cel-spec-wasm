...
void Emit(const Row& row) {
  ...
  cel::CompilerOptions o0;
  o0.optimize_level = 0;
  auto p0 = compiler->Compile(row.source, o0);
  ABSL_CHECK_OK(p0);

  cel::CompilerOptions o2;
  o2.optimize_level = 2;
  ...
  std::printf(
      "%-32s  ast=%5zu B  unopt=%6zu B  opt2=%6zu B  opt2/ast=%4.2fx\n",
...
int main() {
  ...
  auto INT = [](std::string name) -> Row::Decl {
    return {std::move(name), cel::CelType::Int(), "int"};
  };
  auto STR = [](std::string name) -> Row::Decl {
    return {std::move(name), cel::CelType::String(), "string"};
  };
  ...
  std::printf("  cel::Value         %zu B\n", sizeof(cel::Value));
  std::printf("  cel::Activation    %zu B\n", sizeof(cel::Activation));
  std::printf("  cel::Program       %zu B (plus the wasm bytes vector data)\n",
              sizeof(cel::Program));
  std::printf("  cel::Compiler      %zu B (plus the declared-variable vector)\n",
              sizeof(cel::Compiler));
  std::printf("  cel::CompilerOptions  %zu B\n", sizeof(cel::CompilerOptions));
  std::printf("  CelValue (wire)    %zu B  // arena-resident; pinned by static_assert\n",
              sizeof(CelValue));
  return 0;
}
