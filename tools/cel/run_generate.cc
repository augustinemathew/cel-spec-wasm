...
absl::Status RunCppPath(const GenerateOptions& opts,
  ...
  // the IDL has no `Module foo;` directive), via the shared derivation
  // helper — the same one `Plugin::Load` uses, so the generated WIT
  // and the engine's interface lookup cannot drift.  This pairs with
  // the hardcoded `world customfn` in wit_emitter.cc so exports come
  // out as `exports_cel_<module>_fns_*` (m26 §7.5.1).
  const std::string pkg = opts.package_name.empty()
                              ? DeriveWitPackageName(lib.module_name())
                              : opts.package_name;
  const std::string ver = opts.package_version.empty()
                              ? std::string(kWitPackageVersion)
