...
// glance.
//
// We intentionally exclude well-known and wrapper messages from this
// fixture: to reach them, the pool would need to overlay the generated
// pool (MergedDescriptorDatabase), and the resolver path is identical —
// it's just `FindMessageTypeByName(fqn)->FindFieldByName(name)->number()`.
// Those paths are exercised end-to-end via `parse_and_check_test.cc`
// against the real generated pool.
class G2PoolFixture {
 public:
  G2PoolFixture() {
 ...
 private:
  // Adds the nested `Kind` enum with two values; lets the fixture cover
  // the enum code path without pulling in another .proto.
  ...
  // One of every proto scalar + one repeated + one message field.  Field
  // numbers are deliberately non-contiguous so an off-by-one bug in the
  // resolver cannot silently pass.
  static void AddScalarFields(google::protobuf::DescriptorProto* outer) {
    ...
    AddField(outer, "fixed32_field", 12, Field::TYPE_FIXED32,
             Field::LABEL_OPTIONAL);
    // repeated — codegen rejects this as unsupported-in-M3, but the
    // field-number resolver must still populate it; else the codegen
    // error path cannot cite the specific field.
...
// Builds `operand.field` as the root expr.  Assigns `operand_id` to the
// operand and `select_id` to the surrounding SelectExpr so a caller can
// side-map types and inspect annotations keyed by those ids.
...
// One row per proto field shape we expect the resolver to handle.  The
// `type_spec_builder` lambda lets the row specialise the selected
// TypeSpec for the root node without forcing the test harness to know
// every variant.  Coverage intent: one row for each proto wire type plus
// repeated / message / enum, so a regression narrowing the resolver to a
// subset of types fails *some* row.
struct FieldCase {
  const char* label;
...
TEST(PopulateAnnotationsG2Test, ResolvesEveryProtoFieldKind) {
  ...
  for (const auto& row : cases) {
    ...
    // The selected-field repr does not feed the resolver — it depends on
    // operand type only — so we leave the select node's type_map entry
    // absent; the check below only cares about `field_number`.
...
TEST(PopulateAnnotationsG2Test, ResolvesTestOnlySelect) {
  // `has(msg.s_field)` lowers to a SelectExpr with test_only=true.  The
  // field-number resolution must fire regardless of test_only, since
  // codegen needs the number for both `get_field` and (eventually)
  // `has_field` lowering.
  G2PoolFixture fx;
...
TEST(PopulateAnnotationsG2Test, LeavesNonSelectNodesAtZero) {
  // Pure ident AST — no SelectExpr anywhere.  `field_number` must stay at
  // its default sentinel of 0 even when the pool is provided.
  G2PoolFixture fx;
  cel::Expr root;
  root.set_id(42);
...
TEST(PopulateAnnotationsG2Test, UnknownFieldLeavesZero) {
  // The operand is a known message type, but the selected field name does
  // not exist in the descriptor.  The resolver must not crash and must
  // leave `field_number` at 0 so codegen can emit a safe error path.
  G2PoolFixture fx;
  cel::Ast ast = BuildSelectAst(/*operand_id=*/1, /*select_id=*/2, "msg",
                                "does_not_exist");
  ast.mutable_type_map()[1] =
...
TEST(PopulateAnnotationsG2Test, UnknownMessageTypeLeavesZero) {
  // Operand type names a message the pool does not know about.  Must not
  // crash and must leave `field_number` at 0.
  G2PoolFixture fx;
  cel::Ast ast =
...
TEST(PopulateAnnotationsG2Test, NonMessageOperandLeavesZero) {
  // Defensive: if the operand happens to type-check as a non-message (e.g.
  // a bug elsewhere, or a map select from `has(m.k)`), skip silently
  // rather than dereferencing a null descriptor.  This is the runtime
  // complement of the ParseAndCheck-level test that runs `has(m.k)` over
  // a `map<string,int>` operand.
  G2PoolFixture fx;
  cel::Ast ast =
...
TEST(PopulateAnnotationsG2Test, ResolvesNestedSelectChain) {
  // `msg.inner.i64_field` — two nested SelectExprs.  Both must resolve.
  G2PoolFixture fx;
  cel::Expr root;
  root.set_id(3);
  ...
  EXPECT_EQ(annotations.Find(2)->field_number, 5u);  // inner
  EXPECT_EQ(annotations.Find(3)->field_number, 1u);  // i64_field on inner
}

TEST(PopulateAnnotationsG2Test, NullPoolSkipsFieldNumberResolution) {
  // Even when a SelectExpr is present, a null pool must leave
  // `field_number` untouched at its default (0) — the reprs still seed.
  cel::Ast ast =
      BuildSelectAst(/*operand_id=*/1, /*select_id=*/2, "msg", "i64_field");
  ast.mutable_type_map()[1] =
      cel::TypeSpec{cel::MessageTypeSpec("celwasm.testg2.G2Msg")};
