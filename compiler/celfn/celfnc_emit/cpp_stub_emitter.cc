...
std::string AuthorCStruct(const CelfnType& t) {
  ...
  switch (t.kind) {
    ...
    case K::kList: {
      ...
      if (absl::StartsWith(innerv, "customfn_")) innerv.remove_prefix(9);  // strip "customfn_"
    ...
    case K::kMap: {
      ...
      if (absl::StartsWith(kv, "customfn_")) kv.remove_prefix(9);  // strip "customfn_"
      if (absl::EndsWith(kv, "_t")) kv.remove_suffix(2);
      absl::string_view vv = v;
      if (absl::StartsWith(vv, "customfn_")) vv.remove_prefix(9);  // strip "customfn_"
...
absl::StatusOr<std::string> EmitOneExport(const CelfnDecl& d,
  ...
  if (ret_c == Carrier::kScalarValue) {
    absl::SubstituteAndAppend(&out, kExportScalarTpl,
                              ScalarCType(d.return_type), export_name,
                              sig_args, called_fn, call_arg_list);
    return out;
  }
