...
// NOTE: delimiter is `tpl`, NOT `cpp` — the Google .clang-format
// preset reformats `R"cpp(...)cpp"` CONTENTS as C++ (RawStringFormats),
// which mangles emitted-text templates (it broke `$0#include <vector>`
// once).  Emitted-text templates must never use the cpp delimiter.
constexpr absl::string_view kFilePreambleTpl =
    R"tpl(// Generated by `cel generate` — author-facing declarations.
