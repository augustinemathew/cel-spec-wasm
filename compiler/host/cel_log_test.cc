...
class Scratch {
 public:
  Scratch() { bytes_.assign(kMemBytes, 0); }
  ...
  uint32_t WriteUnknownSet(uint32_t desc_off, uint32_t ids_off,
    ...
    for (size_t i = 0; i < ids.size(); ++i) ids_p[i] = ids[i];
...
TEST(CelLogTest, BoolDirectiveRendersTrueFalse) {
  ...
  EXPECT_EQ(DecodeOne(mem, "b=%b", argv_off, 1),
            "[test.cc:17 TestFn] b=false");
}
...
TEST(CelLogTest, MultipleDirectivesIndexArgvSequentially) {
  ...
  EXPECT_EQ(DecodeOne(mem, "%s=%d", argv_off, 2),
            "[test.cc:17 TestFn] ok=7");
}
...
TEST(CelLogTest, StringOutOfRangePrintsOob) {
  ...
  mem.WriteSlot(argv_off, CEL_LOG_TAG_STR, payload);
  EXPECT_EQ(DecodeOne(mem, "x=%s", argv_off, 1),
            "[test.cc:17 TestFn] x=<oob>");
}
