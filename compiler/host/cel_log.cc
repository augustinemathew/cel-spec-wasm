 ...
 public:
  void Emit(absl::string_view file, absl::string_view fn, uint32_t line_no,
    ...
    std::fprintf(stderr, "[%.*s:%u %.*s] %.*s\n",
                 static_cast<int>(file.size()), file.data(), line_no,
                 static_cast<int>(fn.size()), fn.data(),
...
void FormatSpanPayload(absl::Span<const uint8_t> mem, const CelSpan& s,
  ...
  if (kind == "string") {
    absl::string_view sv(reinterpret_cast<const char*>(bytes.data()),
                         bytes.size());
    absl::StrAppendFormat(out, "string(%s)",
                          absl::CEscape(sv));
    return;
  }
...
void FormatError(absl::Span<const uint8_t> mem, uint32_t err_off,
                 std::string* out) {
  if (err_off == 0 ||
      static_cast<uint64_t>(err_off) + 12 > mem.size()) {
  ...
  absl::StrAppendFormat(out, "error(code=%u,\"%s\")", code,
                        absl::CEscape(msg));
}
...
void ApplyDirective(char c, absl::Span<const uint8_t> mem,
                    const ArgvSlot& slot, std::string* out) {
  switch (c) {
    case 's':
...
std::string FormatMessage(absl::Span<const uint8_t> mem, absl::string_view fmt,
  ...
  for (size_t i = 0; i < fmt.size(); ++i) {
    ...
    bool ok = false;
    ArgvSlot slot =
        ReadSlot(mem, argv_ptr + arg_idx * kArgvSlotBytes, &ok);
    if (!ok) {
...
void CapturingCelLogSink::Emit(absl::string_view file, absl::string_view fn,
  ...
  lines_.push_back(absl::StrCat("[", file, ":", line_no, " ", fn, "] ",
                                message));
}

void DecodeCelLog(absl::Span<const uint8_t> linear_memory,
                  const CelLogWireArgs& args, CelLogSink* sink) {
  absl::string_view file = SafeSpan(linear_memory, args.file_ptr, args.file_len);
