...
later milestones) is currently routed through **host trampolines** —
the wasm module imports `cel_timestamp_parse`, `cel_duration_parse`,
…, and each host language (Rust, Go, JS/browser, Python, …) must
re-implement the body, ideally with bit-identical semantics to
cel-cpp's `absl::ParseTime` / `absl::ParseDuration`.
...
Five plausible strategies for any "parser-shaped" runtime function:
  ...
  - **B. Vendor a full library (absl) into the wasm runtime** —
    `wasi-sdk` builds `absl::ParseTime` etc. into `cel_runtime.wasm`;
    no host work needed.
