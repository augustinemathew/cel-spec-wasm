...
| M3  | `m3-proto-and-strings.md`                  | IN PROGRESS  | Proto field reads (`cel_host.get_field`, `has_field`), `has()`, string constants / equality / concat / size. Slices A+B (runtime wiring, host loader, string literals + equality), C (scalar `kIdentExpr`), D (`+`/`==`/`!=`/`size` on string), E (`startsWith`/`endsWith`/`contains`), F (bytes constants + operators), G1 (message params as externref + `$cel_refs` table / wrappers), G2 (`kSelectExpr` → `cel_host.get_field` with scalar payload loads + realistic `Customer` proto e2e fixture) all landed 2026-04-18/19. Remaining: slice G3 (`has(msg.field)` → `cel_host.has_field`), G4 (nested-message select + `cel_host.message_eq`), CLI `--schema` integration. |
