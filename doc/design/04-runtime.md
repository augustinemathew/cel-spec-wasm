...
- **Polymorphic equality** is one `equality_kernel` (`cel_runtime.c`): cross-numeric ladder, same-kind scalar arms (`cel_bool_eq_at_vv` / `cel_string_eq_at_vv` / `cel_bytes_eq_at_vv` / `cel_null_eq_at_vv` / `cel_numeric_eq_at_vv`, exported for direct native-test and WAT-fixture access; the historical per-kind int/uint/double eq+ne and `*_ne` arms were deleted as unreachable from compiled CEL), aggregate dispatchers, `cel_host.cel_message_eq` for messages, and cross-kind → `false` (a value, not an error). `cel_not_equals_at_vv` negates only when the equality result is `CEL_BOOL`, passing 3VL poison/unknown through unchanged.
