...
| R15 | **FIXED.** The loop-cond peephole read accu payload bits without checking kind, so an ERROR accumulator tripped `exists`'s exit early — `[0, 2].exists(x, 2/x == 1)` returned error instead of spec-true. `BoolAccuExit` now gates the `br_if` on `kind == CEL_BOOL` for `exists` and `all` alike. | expr_lower_comprehension.cc (`BoolAccuExit`); e2e `ComprehensionAccuAbsorptionE2ETest` | codegen-lowering #5 |
...
| ID | Discrepancy | Citations | Source |
|----|-------------|-----------|--------|
| R36 ✅ RESOLVED 2026-07-25 (m35 B1) | `engine.h:168-170` claimed AddComponent (now AddPlugin) validates exported FuncType vs decl signature at registration; export resolution is Plan-time, no FuncType comparison exists, arity mismatch surfaces only as a call-time trap. Fixed: AddPlugin's header comment now states the truth (registration validates collisions + parse only; export lookup is Plan-time; no FuncType validation on any path), and the new `Engine::Use` carries the registration-time STATIC export-existence check. | engine.cc:1052-1097,856-861; engine.h AddPlugin/Use doc-comments | eval-public #2 |
