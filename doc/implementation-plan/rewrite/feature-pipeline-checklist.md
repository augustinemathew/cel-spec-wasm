...
### 2.7 New top-level custom section (removed feature-type)

(This section described metadata sections on plugin Component-Model
binaries — the `cel.fns` pattern.  The plugin backend was removed
2026-08-04, m39-component-removal.md.  The surviving rule of thumb:
`//abi:wasm_binary` is THE binary-format layer — any new magic
constant, LEB codec, or section walker outside it is a review
finding; a new custom section on the Program's core module follows
§2.6's emit/decode/consume/test shape with `AppendCustomSection` /
`FindCustomSection`.)
