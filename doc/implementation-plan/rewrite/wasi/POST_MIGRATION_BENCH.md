...
- ~~Compile cel_runtime with `-DCEL_LOG_DISABLED` in opt builds
  ...
  on):
    ...
    - `BM_Eval_ListAt_Arena`: 3 649 → 654 ns (5.6× faster)
    - `BM_Eval_CreateList`: 3 256 → 689 ns (4.7× faster)
    - `BM_Eval_Literal`: 259 → 137 ns (1.9× faster, now below the
      pre-WASI `f4b7157` baseline of 160 ns)
  The per-Eval host roundtrip cost dominated; cutting it puts the
  Eval rows uniformly below the pre-WASI baseline.  Binary-size
  delta, BM_Compile, and BM_Plan are unaffected — CEL_LOG fires
  on the runtime hot path, not the codegen / instantiation path,
  and the wasi-libc instantiation + dlmalloc overhead is still
  paid per Plan (visible as +33% Plan in `bench/README.md`).
