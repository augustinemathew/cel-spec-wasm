...
> has **13 functions** over the `readability-function-size` gate with
> no NOLINTs (`UnpackOneAnyLayer`, `EncodeValue`, `CelListAtImpl`,
> `CelListIterOpenImpl`, `CelMapIterOpenImpl`, `BackingValueEqualsQuery`,
> `SetWrapperInnerValue`, `SetScalarField`, `AppendRepeatedFromCelValue`,
> `AppendRepeatedFromHostListValue`, `InsertArenaMapEntry`,
> `InsertHostMapEntry`, `CelSetFieldImpl`; largest `SetScalarField`).
> These are deferred to the **m33 cel_host cleanup** (the function
> decomposition is m33's scope; see `rewrite/m33-*.md`).  The cheaper
> mechanical cel_host.cc findings (`clang-analyzer-...NullArg` FPs,
> `modernize-use-auto`, `readability-math-missing-parentheses`,
> `misc-use-internal-linkage`, `performance-no-automatic-move`) were
> cleared while landing the m31 static-aggregate work.  `cel_host_test.cc`
> still trips three clang-tidy FPs (a `pro-type-member-init` on an
> already-initialized `flat_hash_map` field, two `unnecessary-value-param`
> on params that are in fact `std::move` sinks, and a
> `prefer-member-initializer` that can't move without a ctor restructure)
> — also m33.  Until a full re-inventory is run (drop the
> `startswith('compiler/')` filter and sweep every first-party role dir),
> treat this doc as historical; the working-set lint (`scripts/lint.sh`)
> is the live gate.
